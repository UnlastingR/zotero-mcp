...
export class MCPSettingsService {
  ...
  private static readonly DEFAULTS: Record<string, any> = {
    ...
    // Doc2X & Bilingual Noise Filter Settings
    "semantic.bilingualFilter": true,
    "semantic.blacklistPatterns":
      '_translated, _zh, _cn, (中文), (翻译), [翻译], [译文], 中文翻译, 译文, <div class="translation">',
  };
  ...
  /**
   * Get bilingual noise filter status
   */
  static isBilingualFilterEnabled(): boolean {
    return this.get("semantic.bilingualFilter") ?? true;
  }

  /**
   * Get bilingual blacklist patterns
   */
  static getBilingualBlacklistPatterns(): string[] {
    const raw = this.get("semantic.blacklistPatterns") || "";
    if (!raw) return [];
    return raw
      .split(/[,,\n]/)
      .map((s: string) => s.trim())
