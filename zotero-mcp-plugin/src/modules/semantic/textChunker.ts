...
export class BilingualNoiseCleaner {
  ...
  static clean(text: string, customPatterns?: string[]): string {
    ...
    try {
      ...
      // 1. Remove HTML translation containers (e.g. <div class="translation">...</div>)
      processed = processed.replace(
        /<div[^>]*class=["'](?:translation|zh|cn)["'][^>]*>[\s\S]*?<\/div>/gi,
        "",
      );
      processed = processed.replace(
        /<span[^>]*class=["'](?:translation|zh|cn)["'][^>]*>[\s\S]*?<\/span>/gi,
        "",
      );

      // 2. Remove Markdown quote blocks with translation tags (e.g. > [译文] ... or > [翻译] ...)
      processed = processed.replace(
        /^>\s*\[(?:译文|翻译|Translation)\][\s\S]*?(?=\n\n|\n[^\>]|$)/gim,
        "",
      );

      // 3. Process line-by-line and section truncation
      const lines = processed.split("\n");
      const resultLines: string[] = [];

      // Compile regex patterns from blacklist
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // Check section truncation headers (e.g. # 中文翻译, ## 译文)
        if (
          /^#+\s*(?:中文翻译|参考译文|译文|Translation\s*\(ZH\)|Translation\s*\(CN\))/i.test(
            trimmed,
          )
        ) {
          if (typeof ztoolkit !== "undefined") {
            ztoolkit.log(
              `[BilingualNoiseCleaner] Truncating translation section at line ${i}: ${trimmed}`,
            );
          }
          break; // Stop parsing remaining translation section
        }

        // Check if line matches any blacklisted pattern
        const isBlacklisted = regexList.some((reg) => reg.test(trimmed));
        if (isBlacklisted) {
          continue; // Skip blacklisted line
        }
      ...
      if (typeof ztoolkit !== "undefined") {
        ztoolkit.log(
          `[BilingualNoiseCleaner] Error during clean: ${e}`,
          "error",
        );
      }
      return text;
...
export class TextQualityPreprocessor {
  ...
  static process(text: string): { text: string; quality: QualityResult } {
    ...
    // 5.5. Clean Doc2X translation noise and blacklisted patterns
    processed = BilingualNoiseCleaner.clean(processed);
