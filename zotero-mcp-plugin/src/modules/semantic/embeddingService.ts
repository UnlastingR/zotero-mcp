...
export class EmbeddingService {
      ...
      if (timeoutSeconds) {
        ...
        if (!isNaN(seconds)) {
          const clamped = Math.min(
            MAX_TIMEOUT_SECONDS,
            Math.max(MIN_TIMEOUT_SECONDS, seconds),
          );
        ...
        if (
          ...
          if (currentBatchSize > 1) {
            ...
            if (oversizedItem && oversizedItem.text.length > MAX_SAFE_LENGTH) {
              ztoolkit.log(
                `[EmbeddingService] Truncating oversized item ${oversizedItem.id} from ${oversizedItem.text.length} to ${MAX_SAFE_LENGTH} chars`,
                "warn",
              );
              const truncatedTexts = [
                oversizedItem.text.substring(0, MAX_SAFE_LENGTH),
              ];
              try {
                const embeddings = await this.callEmbeddingAPI(truncatedTexts);
                const embedding = embeddings[0];
                const lang =
                  oversizedItem.language ||
                  this.detectLanguage(truncatedTexts[0]);
                ...
                if (
                  truncateError instanceof EmbeddingAPIError &&
                ...
                ztoolkit.log(
                  `[EmbeddingService] Truncated item still failed, skipping: ${truncateError}`,
                  "warn",
                );
              }
              itemIndex += 1;
              ...
              ztoolkit.log(
                `[EmbeddingService] Single item too large to process: ${oversizedItem?.id}`,
                "error",
              );
              throw new EmbeddingAPIError(
  ...
  private detectErrorType(
    error: any,
    responseBody?: string,
  ): { type: EmbeddingErrorType; retryAfterMs?: number } {
    ...
    const statusCode =
      error.status || error.statusCode || error.xmlhttp?.status;
    ...
    if (
      fullMsg.includes("413") ||
      fullMsg.includes("payload too large") ||
      fullMsg.includes("request entity too large") ||
      fullMsg.includes("content too large") ||
  ...
  private async callEmbeddingAPI(texts: string[]): Promise<number[][]> {
    ...
    if (cleanTexts.length !== texts.length) {
      ztoolkit.log(
        `[EmbeddingService] Filtered ${texts.length - cleanTexts.length} empty texts, ${cleanTexts.length} remaining`,
        "warn",
      );
    }
    texts = cleanTexts;
    ...
    for (let attempt = 0; attempt < this.config.maxRetries; attempt++) {
      try {
        ...
        if (error instanceof EmbeddingAPIError) {
          ...
          const { type, retryAfterMs } = this.detectErrorType(
            error,
            responseBody,
          );
